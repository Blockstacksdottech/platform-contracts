...
contract('EthicHubLending', function([owner, borrower, investor, investor2, investor3, investor4, localNode, ethicHubTeam, community, arbiter, systemFeesCollector]) {
    ...
    describe('Retrieve contribution with interest', async function() {
        ...
        it('Should return correct platform fees', async function() {
            ...
            const expectedEthicHubTeamBalance = new BN(teamBalance).add(new BN(this.loanParams.totalLendingAmount).mul(new BN(this.loanParams.ethicHubFee)).div(new BN(100)))
        ...
        it('Should return remaining platform fees if inexact', async function() {
            ...
            const teamFinalBalance = await web3.eth.getBalance(ethicHubTeam)
            const expectedEthicHubTeamBalance = new BN(teamBalance).add(new BN(loanParams.totalLendingAmount).mul(new BN(loanParams.ethicHubFee)).div(new BN(100)))
    ...
    describe('Fees collected when project is funded', async function() {
        ...
        it('Should be sent to borrower amount without the fees', async function() {
            ...
            const teamFeesAmount = new BN(this.loanParams.totalLendingAmount).mul(new BN(this.loanParams.ethicHubFee)).div(new BN(100))
    ...
    describe('Calculate fees', async function() {
        it('Should reclaims system fees and team fees before lending period', async function() {
            ...
            const ethicHubTeamBalance = await web3.eth.getBalance(ethicHubTeam)
            await feesLending.reclaimEthicHubTeamFee()
            const expectedEthicHubTeamBalance = new BN(ethicHubTeamBalance).add(new BN(loanParams.totalLendingAmount).mul(new BN(loanParams.ethichubFees)).div(new BN(100)))
    ...
    describe('Fix transfer function to call function', async function() {
        it('Should reclaim team fees', async function() {
            ...
            let actors = {
                'borrower': borrower,
                'localNode': localNode,
                'ethicHubTeam': leakGasContract.address,
                'systemFeesCollector': systemFeesCollector
            }

            let lending = await EthicHubLending.new(
                this.mockStorage.address,
                loanParams,
                actors
            ).should.be.fulfilled
            ...
            const ethicHubTeamBalance = await web3.eth.getBalance(leakGasContract.address)
            let tx = await lending.reclaimEthicHubTeamFee()
            let reclaimEHFeeGasCost = accumulateTxCost(tx, reclaimEHFeeGasCost)
            tx.receipt.gasUsed.should.be.above(100000)
            const expectedEthicHubTeamBalance = new BN(ethicHubTeamBalance).add(new BN(loanParams.totalLendingAmount).mul(new BN(loanParams.ethichubFees)).div(new BN(100)))
            const ethicHubTeamBalanceAfter = await web3.eth.getBalance(leakGasContract.address)
            checkLostinTransactions(expectedEthicHubTeamBalance, ethicHubTeamBalanceAfter)
        })
    })
